...
export function checkBogey(remaining: number): number | null {
  return BOGEY_NUMBERS.includes(remaining) ? remaining : null;
}

type SkillTier = 'beginner' | 'club' | 'good' | 'advanced';

function getSkillTier(avg: number): SkillTier {
...
const saferCheckouts: Record<number, string> = {
  170: 'S20 T18 D20', 167: 'S19 T18 D20', 164: 'S18 T18 D19', 161: 'S17 T20 D17',
  160: 'S20 T20 D20', 158: 'S20 T20 D19', 157: 'S19 T20 D20', 156: 'S20 T20 D18',
  155: 'S19 T20 D19', 154: 'S18 T20 D20', 153: 'S19 T20 D18', 152: 'S20 T20 D16',
  151: 'S17 T18 D20', 150: 'S18 T20 D18',
  100: 'S20 S40 D20', 97: 'S19 S20 D20', 96: 'S20 S20 D18', 95: 'S19 S20 D18',
  80: 'S20 S20 D20',
};

export const checkoutHints: Record<number, string> = {
...
export type SuggestionType = 'scoring' | 'setup' | 'checkout' | 'safety';
...
export function getSuggestion(
  ...
  return getScoringSuggestion(tier, stats, ctx);
}

function getCheckoutSuggestion(score: number, highBustRate: boolean): Suggestion | null {
...
function getSetupSuggestion(score: number, tier: SkillTier, avg: number): Suggestion {
...
function getScoringSuggestion(tier: SkillTier, stats: PlayerStats, ctx: SuggestionCtx): Suggestion {
  if (ctx.round <= 3 && ctx.turnsThisLeg < 3 && stats.first_9_average > 0) {
    const target = getTurnTarget(tier);
    return {
      text: 'First-9 avg: ' + stats.first_9_average + '. Target: ' + target + '+',
      type: 'scoring',
    };
  }
...
function getTurnTarget(tier: SkillTier): number {
  switch (tier) {
...
function getAimArea(tier: SkillTier): string {
  switch (tier) {
...
export interface Preset {
  value: number;
  label: string;
  style: '' | 'max' | 'ton-plus' | 'ton' | 'checkout' | 'miss';
}

export function getPresets(score: number, stats: PlayerStats | null | undefined): Preset[] {
  const avg = (stats && stats.x01_average) || 0;
  const tier = getSkillTier(avg);

  let base: number[];
  switch (tier) {
    case 'advanced': base = [60, 85, 100, 120, 140, 160, 180]; break;
    case 'good': base = [41, 60, 80, 85, 100, 140, 180]; break;
    case 'club': base = [26, 41, 45, 60, 80, 100, 140]; break;
    default: base = [10, 20, 26, 30, 41, 45, 60]; break;
  }
